...
### 8. 唯一索引和普通索引的选择
* 对于普通索引来说，查找到满足条件的第一个记录 ，继续查找下一个记录，直到找到第一个不满足的记录
* 对于唯一索引来说，由于索引定义了唯一性，查找到第一个满足条件.就停止检索
* Innodb读数据是将数据页存入内存，不是单条记录
* 唯一索引需要判断一致性，需要存入内存判断，不需要 change_buffer
* 更新记录
这个记录在内存中
...
刷新脏页的情况
1. 当innoDB的 redo log 写满了，会停止更新，腾出一部分空间，需要把所有腾出的空间这段的脏页全部flush到磁盘
...
1. 这个参数设置为 OFF 表示的是，表的数据放在系统共享表空间,
2. 这个参数设置为 ON 表示的是，每个 InnoDB 表数据存储在一个以 .ibd 为后缀的文件中
* 删除行只是删除数据，行的空间不变，标记为可复用
* 不止是删除数据会造成空洞，插入数据也会。
* 创建表可以释放空间
